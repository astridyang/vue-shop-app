		...
		<el-tree :data="tableData" :props="{ children: 'child', label: 'name' }" node-key="id" v-loading="loading"
			...
			<template #default="{ node, data }">
				<div class="custom-tree-node">
					...
					<div class="ml-auto">
						<el-switch :model-value="data.status" :active-value="1" :inactive-value="0" @change.stop="handleUpdateStatus($event,data)">
						</el-switch>
						<el-button type="primary" size="small" text @click="handleEdit(data)">Update</el-button>
						<el-button type="primary" size="small" text @click="handleAdd(data.id)">Add</el-button>
						<!--todo stop-->
						<span @click.stop="()=>{}">
			...
			<el-form :model="form" ref="formRef" :rules="rules" label-width="120px" :inline="false">
				<el-form-item label="Parent Menu" prop="rule_id">
					<el-cascader v-model="form.rule_id" :options="options"
					             :props="{ value: 'id', children: 'child', label: 'name', checkStrictly: true, emitPath: false }"
					             clearable
...
import {createRule, deleteRule, updateRule, updateRuleStatus} from '~/api/rule';
